...
def train(options, model):
    ...
    for j in range(options.epoch):
        ...
        for i_batch, sample_batch in enumerate(tqdm(train_dataloader)):
            ...
            final_preds = torch.ones(options.bt_siz,maxlen,41379)
            final_lmpreds = []
            ...
            target_toks = final_utters.ne(41378).long().sum().data
        ...
        with open("loss.txt","a") as f:
            f.write("train_loss"+str(tr_loss/num_words)+"valid_loss"+str(vl_loss))
        #best_vl_loss = vl_loss
        #patience = 0
        #else:
        #    patience += 1
...
def main():
    ...
    _dict_file = 'douban_entity.dict.pkl'
    # we use a common dict for all test, train and validation
